...
class AuthService  {
    ...
    async SignUp(DatafromBody){
        const {first_Name, last_Name, password, email, phone_Number, father_Name, mother_Name, address} = DatafromBody
...
async Login(DatafromBody) {
    ...
    const checkActive = await this.Model.findById({_id : findemail._id}).where({active : true}).lean()
    if(!checkActive){
        return {
